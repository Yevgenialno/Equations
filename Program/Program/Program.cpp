...
void assemble_tree(int i)
{
	while(i != 0)
	{
		...
		for(int k = 0; k <= strlen(input[i/3]); k++)
			input[i/3][k] = '\0';
		if(strlen(input[i]) > 1) strcat_s(input[i/3], "(");
		strcat_s(input[i/3], input[i]);
		if(strlen(input[i]) > 1) strcat_s(input[i/3], ")");
		strcat_s(input[i/3], input[i+1]);
		if(strlen(input[i+2]) > 1) strcat_s(input[i/3], "(");
		strcat_s(input[i/3], input[i+2]);
		if(strlen(input[i+2]) > 1) strcat_s(input[i/3], ")");
		i=i/3;
	}
}
...
void unite(int j)
{
	for(int t = 0; input[0][t] != '\0'; t++)
		input[0][t] = '\0';
	if(strlen(answers[j][0]) > 2) strcat_s(input[0], answers[j][0]);
	if(strlen(answers[j][1]) > 5)
		strcat_s(input[0], answers[j][1]);
	else
		if(is_X_there) 
			if(strlen(answers[j][0]) > 2) strcat_s(input[0], "+X"); else strcat_s(input[0], "X");
		if(strlen(answers[j][2]) > 6)
			strcat_s(input[0], answers[j][2]);
		else
			if(is_addition_to_X_there)
				if((strlen(answers[j][0]) > 2)||(strlen(answers[j][2]) > 6)) strcat_s(input[0], "+X^"); else strcat_s(input[0], "X^");
	std::cout<<input[0]<<"=0\n";
}
...
bool IsCorrect(char *q)
{
	...
	int n = 0, n0 = 0, equal_signgs_count = 0;
	char s[2], s1[2], s0[2];
	...
	for(char *p = q; *p; p++)
	{
		...
		if((equal_signgs_count == 2) || (n0 == 3) || (n < 0))
		{
			cout << equal_signgs_count << n0 << n;
			return false;
		}
							...
							*(temp + l + 1) = '\0';
							...
							for(long int i = 2; i < level - 3; i+=3) 
							{
								if(input[i][0] == '&') //ubiraem &
								{
									for(int k = 0; k <= strlen(input[i/3]); k++)
										input[i/3][k] = '\0';
									strcat_s(input[i/3], "(");
									if(strlen(input[i-1]) > 1) strcat_s(input[i/3], "(");
									strcat_s(input[i/3], input[i-1]);
									if(strlen(input[i-1]) > 1) strcat_s(input[i/3], ")");
									strcat_s(input[i/3], "-");
									if(strlen(input[i+1]) > 1) strcat_s(input[i/3], "(");
							...
							for(long int i = 2; i < level - 3; i+=3) 
							{
								if((input[i][0] == '^') && (strlen(input[i-1]) > 1)) //ubiraem ()^
								{
									...
									else
									{
										...
										input[i][0] = '\0';
										strcat_s(input[i], "(");
										if(strlen(input[3*i+1]) > 1) strcat_s(input[i], "(");
										strcat_s(input[i], input[3*i+1]);
										if(strlen(input[3*i+1]) > 1) strcat_s(input[i], ")");
										strcat_s(input[i], "^");
										strcat_s(input[i], ")");
										strcat_s(input[i], input[3*i+2]);
										strcat_s(input[i], "(");
										if(strlen(input[3*i+3]) > 1) strcat_s(input[i], "(");
										strcat_s(input[i], input[3*i+3]);
										if(strlen(input[3*i+3]) > 1) strcat_s(input[i], ")");
										strcat_s(input[i], "^");
										strcat_s(input[i], ")");
									}
							...
							for(int i = 2; i <= (level-3)/3; i+=3) //ubiraem vneshnie peresecheniya
							{
								if((input[i][0] == '*')&&(input[3*(i-1)+2][0] == '+'))
								{
									...
									if(strlen(input[i+1]) > 1) strcat_s(input[i/3], ")");
									strcat_s(input[i/3], "*");
									if(strlen(input[3*i-2]) > 1) strcat_s(input[i/3], "(");
									strcat_s(input[i/3], input[3*i-2]);
									if(strlen(input[3*i-2]) > 1) strcat_s(input[i/3], ")");
									strcat_s(input[i/3], ")");
									strcat_s(input[i/3], "+");
									strcat_s(input[i/3], "(");
									...
									if(strlen(input[i+1]) > 1) strcat_s(input[i/3], ")");
									strcat_s(input[i/3], "*");
									if(strlen(input[3*i]) > 1) strcat_s(input[i/3], "(");
									strcat_s(input[i/3], input[3*i]);
									if(strlen(input[3*i]) > 1) strcat_s(input[i/3], ")");
									strcat_s(input[i/3], ")");
									i=i/3;
									assemble_tree(i);
								...
								if((input[i][0] == '*')&&(input[3*(i+1)+2][0] == '+'))
								{
									...
									if(strlen(input[i-1]) > 1) strcat_s(input[i/3], ")");
									strcat_s(input[i/3], "*");
									if(strlen(input[3*i+4]) > 1) strcat_s(input[i/3], "(");
									strcat_s(input[i/3], input[3*i+4]);
									if(strlen(input[3*i+4]) > 1) strcat_s(input[i/3], ")");
									strcat_s(input[i/3], ")");
									strcat_s(input[i/3], "+");
									strcat_s(input[i/3], "(");
									...
									if(strlen(input[i-1]) > 1) strcat_s(input[i/3], ")");
									strcat_s(input[i/3], "*");
									if(strlen(input[3*i + 6]) > 1) strcat_s(input[i/3], "(");
									strcat_s(input[i/3], input[3*i + 6]);
									if(strlen(input[3*i + 6]) > 1) strcat_s(input[i/3], ")");
									strcat_s(input[i/3], ")");
									i=i/3;
									assemble_tree(i);
							...
							for(int u = 0; u < 3; u++)
							{
								if((strlen(globalAns[u]) > 0) && (strlen(answers[j][u]) > 0))
									strcat_s(globalAns[u], "+");
								int inc;
								if(u == 0) inc = 0;
								if(u == 1) inc = 3;
								if(u == 2) inc = 4;
								strcat_s(globalAns[u], answers[j][u] + inc);
